...
+ 常量池结束后端两个字节是访问标志，访问标志是一个
+ 用于表示某个类或者接口的访问权限及基础属性。
+ 是类还是接口、是否定义public、是否abstract、是否final等



##### 类索引、父类索引与接口索引集合

+ 类索引、父类索引与接口索引集合按照顺序排列在访问带标志后面，描述了类的继承和接口实现方式。
+ 类索引确定当前类的全限定名（例如org/fenixsoft/clazz/TestClass;）、父类索引去定了这个类的父类的全限定名，接口索引集合确定了当前类实现了那些接口
+ 全限定名（例如org/fenixsoft/clazz/TestClass;）
+ 

##### 字段表集合

+ 描述接口或者类中声明的变量，字段包括类级变量和实例级变量，但是不包含方法内补生命的变量
...
+ 将字节流的静态存储结构转化为方法区中的运行时数据结构
+ 在堆中生成一个java.lang.Class对象，作为方法区这些数据访问的入口

#### 验证

+ 文件格式验证：魔数是否正确、主次版本是不是在当前虚拟机处理范围之内、常量池中的常量是不是有不支持的类型

+ 元数据验证：对字节码描述的信息进行语义分析，要求符合java语言的规范，例如是不是继承了不能被继承的类

+ 字节码验证：进行数据流和控制流的分析，对类的方法体进行校验，保证方法在运行时不会危害虚拟机安全

+ 符号引用验证：发生在解析阶段中产生，将符号引用转化为直接引用的时候。验证符号引用中通过字符串描述的全限定名是否能找到对应的类等，他的目的是

  为了保证解析动作正常执行。

#### 准备

+ 正式的为类变量分配内存，并设置类变量初始值。（只有类中的静态变量进行初始化），将基本类型赋值成对应的零值，将引用类型设置成null
+ 如果是静态常量就可以直接赋值成对应的值
...
+ 代码加密，类层次划分，热部署
#### 判断两个类是否相等
+ Class对象的equals()、isAssignableFrom()、isInstance()
#### 双亲委派机制
...
```ja
// 互斥同步通过线程的阻塞和唤醒实现同步，成为阻塞同步，属于悲观锁
```



##### synchronize
+ 被synchronize修饰的代码片段编译后,
  会在同步块前后生成monitorenter和monitorexit两个字节码指令，
  这两格字节码的指令都需要一个引用类型来指明锁定和解锁的对象。
  如果没有明确指明一个对象，那就取对象实例或者类Class对象作为锁对象。
  执行monitorenter时，会尝试获取对象的锁，如果当前对象的没有被锁定，则当前线程已经拥有了

+ java阻塞和唤醒线程需要操作系统的配合，这样就需要将用户态向核心态转换，

##### synchronize和R'e'entrantLock进行比较
...
```java
// 基于冲突检测的乐观并发策略，先进行操作，如果没有其他线程征用共享数据，就操作成功。如果发生冲突，那就使用不久措施。 属于乐观锁。非同步阻塞
// 需要基于硬件的操作和冲突检测的原子性
```



+ 处理器乐观锁原子性指令有：测试并设置、获取并增加、交换、比较并交换、加载链接/条件存储

#### CAS

+ 可以使用JUC的原子类来进行操作
+ 不能避免ABA问题，如果业务需要的话可以使用AtomicStampedReference

#### 线程本地存储

+ 每一个ThreadLocal讲对象都有一个ThreadLocalMap对象，以ThreadLocalHashCode为建，线程本地变量为值得形式存储
...
+ 用于解决互斥锁阻塞和唤醒线程操作需要转入内核态性能问题，
+ 当有线程请求已经有线程持有锁的资源的时候，让请求的线程等待（忙循环）不进入阻塞，
+ jdk1.6引入了自适应的自旋锁，自选时间不固定，根据前一次的同一个上锁的自旋时间及锁的拥有者的状态来决定。在同一个锁对象上，如果自选等待刚刚通过，并且持有锁的线程正在运行中，那么虚拟机会认为自旋很有可能再次成功，会增加自选等待的时间，反之，对于某个锁自旋很少成功过，那么虚拟就就可以省略掉自选过程。

#### 锁清除

+ 锁消除的判断依据逃逸分析，判断一段代码，如果在堆上的数据，都不会逃逸出去被其他线程访问到，就可以把它当做栈上的数据对待，认为他是线程私有的，就不需要加锁

#### 锁粗化

+ 虚拟机会优化连续对一个i对象的反复加锁和解锁的过程，只加一次锁

#### 轻量级锁

##### 对象头

+ 存储对象自身运行时数据（如果是32位虚拟机），如哈希码（25bit），GC分代年龄（4bit）、锁标志位（2bit）、
+ 指向方法区对象对象类型数据的指针，如果是数组对象的话还会存放数组长度

在没有多线程竞争的前提下，减少传统的重量级锁得使用操作系统互斥量产生的性能消耗。

+ 加锁的过程：

  + 当进入到同步代码块的时候，如果当前同步对象没有被锁定，虚拟机将在栈帧中建立一个锁记录的空间来存储锁对象mark word的拷贝，

  + 使用将锁对象的mark word 替换成栈帧锁记录的指针，同时mark word的锁标志位将转变成轻量级锁定
  + 如果操作失败了，首先检查锁对象的mark word是否指向当前线程的栈帧，如果有那么就说明当前线程已经获得了锁。否则说明这个锁对象被其他线程抢占了。当前的锁就要膨胀成重量级锁，将标志位改成重量级锁标志，mark word重存储的就是指向重量级锁的指针

#### 偏向锁（1.6）

+ 加锁方式同轻量级锁，如果操作失败第二次将不需要CAS
